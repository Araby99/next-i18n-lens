...
export const EditorPanel = ({
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!LOCALE_RE.test(code)) {
      setLocalError('Locale format must be 2-3 letters, optionally followed by a region (e.g., fr or es-MX).');
      return;
    }
      ...
      {isManageOpen ? (
        <div className="flex-1 flex flex-col p-6 overflow-y-auto space-y-6">
          ...
          <form onSubmit={handleAdd} className="space-y-2.5">
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Add New Locale</label>
            <div className="flex items-center gap-2">
              <input
                type="text"
                value={newLocaleCode}
          ...
          <div className="flex-1 flex flex-col space-y-2">
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Active Locales</label>
                ...
                >
                  {editingLocale === l ? (
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                      <span className="text-[10px] font-semibold text-rose-400">Confirm deletion? This deletes the translation file.</span>
                    ...
                    <>
                      <div className="flex flex-col min-w-0">
                        <span className="text-xs font-semibold text-slate-200 truncate">{getLocaleLabel(l)}</span>
                        {l === locale && <span className="text-[9px] text-indigo-400 font-bold tracking-wide mt-0.5">ACTIVE WORKSPACE</span>}
                      </div>
